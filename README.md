...
Pseudorandom number generators produce a series of random numbers by performing an operation on the previous value, meaning an initial value is needed to start the sequence. The term 'random seeds' refers to these initial values. Usually, the system's current time is used as a random seed, but it can be customised. The reason the number generators are ‘pseudorandom’ is because they are determined by the seed. Therefore, if the same seed is used, the same sequence of ‘random’ numbers will be produced. This means simulations can be made reproducible by setting the seed as a given number.  
