...
Pseudo random number generators produce random numbers by performing an operation on the previous value generated in the series $^1$. This means an initial value, known as a random seed, is needed to start the sequence $^2$. Usually, the system's current time is used as a random seed, but it can be customised. The reason the number generators are ‘pseudo random’ is because they are determined by the seed. Therefore, if the same seed is used, the same sequence of ‘random’ numbers will be produced. This means simulations can be made reproducible by setting the seed as a given number.  
...
1. Singla Y. Pseudo Random Number Generator (PRNG) - GeeksforGeeks [Internet]. GeeksforGeeks. 2017. Available from: https://www.geeksforgeeks.org/pseudo-random-number-generator-prng/


2. Set Seed in R with set.seed() function [Internet]. R CODER. 2020. Available from: https://r-coder.com/set-seed-r/
